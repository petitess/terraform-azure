...
resource "azurerm_storage_account" "st_map" {
  for_each = var.st_map
  depends_on = [
    azurerm_resource_group.st_map,
    azurerm_resource_group.func
  ]
  name                            = each.value.name
  tags                            = local.tags
  resource_group_name             = each.value.rg_name == null ? azurerm_resource_group.st_map_common.name : (startswith(each.value.rg_name, "ex=rg") ? replace(each.value.rg_name, "ex=", "") : each.value.rg_name)
  location                        = local.location
...
resource "azurerm_storage_queue" "st_map" {
  ...
  storage_account_id = azurerm_storage_account.st_map[each.value.st_name].id
...
resource "azurerm_private_endpoint" "st_map_pep" {
  ...
  location                      = local.location
  tags                          = local.tags
  resource_group_name           = azurerm_storage_account.st_map[each.value.st_key].resource_group_name
