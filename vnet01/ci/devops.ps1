...
$Org = "https://dev.azure.com/XCloud"
$Project = "XCare"
$Repo = 'x-infra'
az repos list --query [].name -o tsv

foreach ($Product in $Products | Where-Object { $_ -notmatch "devops.ps1" -and $_ -like "ci-*.yml" }) {
    if (!(az pipelines show --org $Org --project $Project  --name "Infra-$System - CI" 2> $null)) {
        $Product -like "ci-*.yml"
        az pipelines create --org $Org --project $Project --name "Infra-$System - CI" --yml-path "ci/$Product" --folder-path $System --branch main --repository $Repo --repository-type 'tfsgit' --query definition.uri
...
foreach ($Product in $Products | Where-Object { $_ -notmatch "devops.ps1" -and $_ -like "cd-*.yml" }) {
    ...
    if (!(az pipelines show --org $Org --project $Project  --name "Infra-$System - CD" 2> $null)) {
        az pipelines create --org $Org --project $Project --name "Infra-$System - CD" --yml-path "ci/$Product" --folder-path $System --branch main --repository $Repo --repository-type 'tfsgit' --query definition.uri
