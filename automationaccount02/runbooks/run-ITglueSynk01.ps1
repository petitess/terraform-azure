...
$apiKey = Get-AzKeyVaultSecret -VaultName "kv-xxx-infra-prod-01" -Name "ItGlueApiKey" -AsPlainText
Set-ITGlueAPIKey -Api_Key $apiKey
Set-ITGlueBaseURI "https://api.eu.itglue.com"

$organizationId = Get-AzKeyVaultSecret -VaultName "kv-xxx-infra-prod-01" -Name "ItGlueOrgId" -AsPlainText
$CET = (Get-date).AddHours(2)
$synk = "Last_Synk: $(Get-Date $CET -Format "dd/MM/yyyy HH:mm")"
$SubId = (Get-AzSubscription -SubscriptionName 'sub-xxx-prod-01').Id
$VMs = Get-AzVM -Status #| Where-Object { $_.Name -match 'vmvdaprod03' -or $_.Name -match 'vmvdaprod04' }

$VMs | ForEach-Object {
    $Nic = Get-AzNetworkInterface -Name "$($_.Name)*" 
    $OsId = if ($_.StorageProfile.ImageReference.Sku -match '2022-datacenter') { '223' } else {if ($_.Name -match 'vmvda') { '205' } else{ if ($_.StorageProfile.ImageReference.Publisher -match 'canonical' -or $_.StorageProfile.ImageReference.Publisher -match 'citrix') { '61' } else { '36' }}}
